...
void Database_close()
{
	...
	if(conn) {
		if(conn->db && conn->db->rows) {
			// goes through each Address
			// frees each name and email individually
			for(i = 0; i < conn->db->max_rows; i++) {
				// Address pointer curr points
				// to current Address within the database
				// row (array of pointers to Addresses)
